							      ...
							      long int valorDen)
{
	...
	/* copia os long int passado como parâmetro */
...
long double  Modulo_ (MeuRacional_t   const * const me)
{
	long double real = ((long double) GetNum_(me))/((long double) GetDen_(me));
	return real; 
}
...
static  char * imprime_  (Numero_t const * const  me)
{
	...
	{   sprintf(buffer, "%ld/%ld",GetNum_((MeuRacional_pt) me)*(-1), GetDen_((MeuRacional_pt) me)*(-1) );
	}
